...
header("Content-Disposition: attachment; filename=CT_FDA_AdvE_{$type}_pid_{$project_id}_{$todays_date}.txt");
$out = fopen('php://output', 'w');

fputcsv($out, $raw_header,chr(9)) or die("cannot write header");
foreach ($result_array as $fields) {
    if ($fields['adverse_event_type'] == $type) {

        fputcsv($out, $fields, chr(9)) or die("cannot write array");
    }
}
fclose($out);
unset($out);
